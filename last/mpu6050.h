...
class MPU6050 : public TwoWire {
private:
  uint8_t __address__;
  uint32_t __clk_frequency__;

public:
  uint8_t __accelerometer_range__;
  uint8_t __gyroscope_range__;
  uint8_t __gyro_and_acc_dlpf_config__;
  uint16_t __gyroscope_scale_factor__;
  uint16_t __accelerometer_scale_factor__;

  MPU6050(uint8_t address, uint32_t clk_frequency) {
    if (MPU6050_ADDRESS_AD0_LOW == address) {
      __address__ = MPU6050_ADDRESS_AD0_LOW;
      if (clk_frequency == 400000) {
        __clk_frequency__ = clk_frequency;
      } else {
        __clk_frequency__ = 100000;
      }
    } else if (MPU6050_ADDRESS_AD0_HIGH == address) {
      __address__ = MPU6050_ADDRESS_AD0_HIGH;
      if (clk_frequency == 400000) {
        __clk_frequency__ = clk_frequency;
      } else {
        __clk_frequency__ = 100000;
      ...
      __address__ = MPU6050_DEFAULT_ADDRESS;
      if (clk_frequency == 400000) {
        __clk_frequency__ = clk_frequency;
      } else {
        __clk_frequency__ = 100000;
